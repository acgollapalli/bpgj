...
get_mesh_data :: proc(mesh_name: string, alloc := context.allocator) -> (hxa.File, []Vertex, [dynamic]Index) {
        ...
        // NOTE(caleb): This should only be done in preprocessing, meshes that ship should have triangles in threes
        {
            ...
            if index < 0 {
                if j == 2 {
                    ...
                    fmt.assertf(j == 3, "We only support primitives of triangles and quads, not %v, in %v", index , index_stack[: i + 1], j)
                    num_quads += 1
